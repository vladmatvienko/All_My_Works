...
class SyntheticDataGenerator(QWidget):
    ...
    def init_ui(self):
        ...
        description_label = QLabel(
            "Раздел «Генерация» предоставляет возможность сгенерировать файл .csv через выбор параметров."
        )
    ...
    def update_columns_table(self):
        ...
        for row_idx, config in enumerate(self.column_configs):
            ...
            name_item.setBackground(Qt.GlobalColor.transparent)
            name_item.setForeground(Qt.GlobalColor.white)
            self.columns_table.setItem(row_idx, 0, name_item)
...
class ColumnParamsDialog(QDialog):
    ...
    def get_config(self):
        ...
        if mode == 'constant':
            ...
            self.config['params']['prompt'] = self.prompt_input.text()
