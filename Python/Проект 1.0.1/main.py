from PySide6.QtWidgets import QApplication, QSplitter, QPlainTextEdit, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QTabWidget, QFileDialog, QTableWidget, QTableWidgetItem, QCheckBox, QDialog, QListWidget, QListWidgetItem, QComboBox
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Хедер
        header_layout = QVBoxLayout()  # Используем QVBoxLayout для вертикального расположения

        # Контейнер для логотипа (размещаем в верхней части)
        logo_container = QWidget()
        logo_layout = QHBoxLayout()
        logo_layout.setAlignment(Qt.AlignLeft | Qt.AlignTop)  # Выравнивание логотипа по левому верхнему углу
        logo_label = QLabel()
        logo_label.setPixmap(QIcon("logotype.png").pixmap(200, 50))
        logo_layout.addWidget(logo_label)
        logo_container.setLayout(logo_layout)

        # Добавляем логотип в верхнюю часть header_layout
        header_layout.addWidget(logo_container)

        # Создаем контейнер для вкладок и добавляем его под логотипом
        ...
        self.tab_widget.addTab(self.generation_tab, "Генерация")
    ...
    def replace_tab_content(self, tab_index, new_widget):
        self.tab_widget.removeTab(tab_index)
        self.tab_widget.insertTab(tab_index, new_widget, "Анализ")
...
class AnalysisTab(QWidget):
    def __init__(self, main_window=None):
        ...
        # Добавление поля для вывода логов
        self.log_label = QLabel("")
        self.log_label.setStyleSheet("color: white;")  # Стиль для белого текста на темном фоне
        file_form_layout.addWidget(self.log_label)
    ...
    def validate_file(self):
        ...
        # Используем бэкенд-функцию для проверки файла
        ...
        try:
            if loader.validate_structure():
                self.log_label.setText(f"Файл прошел валидацию.")
                logging.info("Структура файла валидна.")
                self.import_button.setEnabled(True)
            else:
                self.log_label.setText("Ошибка при валидации файла.")
                logging.error("Структура файла некорректна.")
                self.import_button.setEnabled(False)
        except Exception as e:
            self.log_label.setText(f"Произошла ошибка: {str(e)}")
            logging.error(f"Ошибка при валидации файла: {e}")
            self.import_button.setEnabled(False)
    ...
    def import_file(self):
        ...
        try:
            if loader.load_data():
                self.log_label.setText("Файл успешно загружен.")
                ...
                if column_dialog.exec():
                    ...
                    analysis_page = AnalysisDataPage(selected_columns, loader)
                    self.main_window.replace_tab_content(0, analysis_page)
            else:
                self.log_label.setText("Ошибка при загрузке файла.")
...
class DataRegistry(QWidget):
    ...
    def init_ui(self):
        ...
        sidebar_widget.setMinimumWidth(200)  # Устанавливаем минимальную ширину сайдбара
        ...
        # Перенаправление print() в output_box
        redirect_print_to_output_box(self.output_box)
        
              
        # Добавляем таблицу и результаты в один виджет
        content_widget = QWidget()
        content_layout = QVBoxLayout(content_widget)
        content_layout.addWidget(self.table_widget)
        
        # Добавляем контент в разделитель
        splitter.addWidget(content_widget)
        
        # Добавляем разделитель в основной лэйаут
    ...
    def calculate_skewness(self):
        ...
        if dialog.exec():
            ...
            if skew_dict:
                ResultsDisplayDialog(title="Асимметрия распределения", results=skew_dict).exec()
    ...
    def calculate_kurtosis(self):
        ...
        if dialog.exec():
            ...
            if kurt_dict:
                ResultsDisplayDialog(title="Эксцесс распределения", results=kurt_dict).exec()
    ...
    def calculate_category_percentages(self):
        ...
        if dialog.exec():
            ...
            if result:
                ResultsDisplayDialog(title="Процентное соотношение уникальных значений", results=result).exec()
    ...
    def count_missing_values(self):
        ...
        if dialog.exec():
            ...
            # Открываем диалоговое окно для отображения результатов
            ResultsDisplayDialog(title="Количество пропущенных значений", results=missing).exec()
    ...
    def one_hot_encode(self):
        ...
        if column_dialog.exec():
            ...
            try:
                ...
                self.loader.active_data = data  # Сохраняем измененные данные

                # Обновляем список столбцов (после кодирования появятся новые)
                new_columns = list(data.columns)  # Получаем обновлённые названия колонок
                self.selected_columns = new_columns  # Обновляем текущий список выбранных столбцов

                # Обновляем таблицу с новыми данными и заголовками
                self.table_widget.setColumnCount(len(new_columns))
...
def redirect_print_to_output_box(output_box: QPlainTextEdit):
    class EmittingStream(StringIO):
        ...
        def write(self, text):
            if text.strip():  # Игнорируем пустые строки
                self.output_box.appendPlainText(text.strip())

        def flush(self):
            pass

    stream = EmittingStream(output_box)
    sys.stdout = stream
    return stream
...
class ResultsDisplayDialog(QDialog):
    def __init__(self, title="Результаты", results=None):
        super().__init__()
        self.setWindowTitle(title)
    ...
    def display_results(self, results):
        ...
        if isinstance(results, dict):
            # Если результаты — словарь
            self.table_widget.setColumnCount(2)
            self.table_widget.setHorizontalHeaderLabels(["Показатель", "Значение"])
            ...
            for key, value in results.items():
                ...
                self.table_widget.setItem(row_idx, 0, item_key)
                self.table_widget.setItem(row_idx, 1, item_value)
                row_idx += 1

            self.table_widget.resizeColumnsToContents()
        elif isinstance(results, pd.DataFrame):
            # Если результаты — DataFrame
            self.table_widget.setColumnCount(len(results.columns))
            self.table_widget.setHorizontalHeaderLabels(results.columns.tolist())
            self.table_widget.setRowCount(len(results))

            for row_idx, row_data in enumerate(results.itertuples()):
                for col_idx, value in enumerate(row_data[1:]):  # Пропускаем индекс
                    item = QTableWidgetItem(str(value))
                    self.table_widget.setItem(row_idx, col_idx, item)

            self.table_widget.resizeColumnsToContents()
        else:
            logging.error("Неподдерживаемый тип результата для отображения.")
