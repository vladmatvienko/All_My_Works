...
- When writing code, use C++ idioms like RAII. The "single exit point" rule (single return), required in C projects, is not mandatory in C++ thanks to automatic resource management through destructors.
- Implement the matrix as an `S21Matrix` class;
- Use only the `double** matrix_`, `int rows_` and `int cols_` fields as private;
