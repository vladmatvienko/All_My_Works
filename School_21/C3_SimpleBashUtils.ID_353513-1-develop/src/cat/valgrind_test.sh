...
valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --verbose --log-file=valgrind_out.txt ./s21_cat s21_cat.c test_valgrind.txt
...
# ./s21_cat "test" test_valgrind.txt: команда для тестирования (попытка открыть test.txt)
